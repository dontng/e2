...
def _bg_state_worker():
    """后台每 3 秒刷新 _live_state；远程模式下每轮先 pull 再读。"""
    global _live_state, _pull_err
    while True:
        try:
            if not local_daemon():
                err = pull()
...
def main():
    global _live_state
    # 服务器启动前先同步填好初始状态（跳过 pull），避免首次请求阻塞等待后台线程。
    try:
        with _live_lock:
            _live_state = get_state()
    except Exception:
        pass

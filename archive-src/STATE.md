...
> [!NOTE]
> 这是截至 Day 92 的历史快照，只用于追溯旧风险和旧 T1 设计，不再作为活动状态入口。现行四类专项、训练与复测记录统一从 [`../t1/README.md`](../t1/README.md) 进入。
...
| 当前周 | 2026-W31 |
| 最近 7 天完成 | 7 / 7 |
| 当前 T0 范围 | `0518-day30` 到 `0729-day92` |
| 下一次打开 | 新增或批改 Day 93 |
...
| 请求—执行动作链 | high | Day 91 `asking that it identify actions` | 见到 `ask/request/urge` 后保留“谁被要求做什么”，直到从句核心动词落地 |
| 并列名词化建议 | high | Day 92 `calls for A, B, C and D` | 长清单先逐项回挂同一谓语；每项内部再找修饰、目的和执行者 |
...
1. `ask/request/urge` 后先留下“谁被要求做什么”，直到从句里的核心动作出现。
2. `calls for A, B, C and D` 先逐项圈出 A—D，再分别读每项的内部修饰。
3. 熟动词与 `on/through/to bear` 等成组出现时，直译发怪就先恢复资源如何作用于目标。
...
| 仍然活跃 | 请求后的动作期待、并列建议回挂、长句主干、从句挂载、熟词固定结构的义项迁移 |
| 下次验证 | 能否在请求/建议动词后锁定核心动作，并把长并列项逐一挂回同一谓语 |
...
| 请求动词后的执行动作槽丢失 | high | Day 91 `asking that it identify actions` | 先补全“要求 AAAS 找出什么”，再处理后续名单和目的 |
| bring A to bear on B 被按“背负”拆读 | high | Day 92 `bring their learning to bear on challenges` | 先标 A 是资源、B 是目标，整体落成“把 A 用于 B” |

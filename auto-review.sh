...
学员稳定档位：首译约3.5–4.5/10；独立读句时常卡在多义词、介词方向、主干找错、语义极性搞反、长句读半截。

文件路径：$file

**约束：只允许编辑 $file 这一个文件，不得读取或修改任何其他 source 文件。**

参考语气与深度：$REPO_DIR/src/may/0528-day39.md（section 结构以本任务为准）
...
4. 填写「## Vocab」：
   ...
   - 每个词条**恰好 3 条例句**，禁止从原句截取；用心造句，难度贴近原句（句长、从句层数±20%），英二真题语感
   - 三例句分工固定（形神兼具：李延隆式教案分工 + vocabulary.com 式活句感）：
     ① **扣原句** — 同骨架重演原句中该词的位置与搭配，可换场景/主语，禁止只换专有名词
     ② **考纲迁移** — 英二常见场景中的活句，像评论/报道里会出现的句子
     ③ **防坑** — 针对本次批改 [n] 或采分点✗，拆学员具体误译（形近词、极性反了、介词转向、程度偏差等）；可用 not...but / instead of / rather than，但须像正常英文
   - 词条格式：
   \`\`\`
...
【项目立意】
学员首译稳定约3.5–4.5/10。批改中的英二例句学员单独读仍会懵；你的任务是把每一句拆到他能跟上的程度，
让他读完 fool 就等于消化了今天的阅读训练量。讲解深度固定按此档位，不随当天分数浮动。

句子文件：$file
fool 文件：$fool_path

风格锚：$REPO_DIR/fool/june/0606-day47-fool.md 中 fool-01 的密度与白话程度（逐步拆解，不堆术语）。
...
对每个句子做四步拆解（全文质量一致，禁止后段缩略）：

**Step 1 扫词**：有分量的词 — 词根最多一行 + 核心意象一行中文；简单句极简。

**Step 2 扫小品词和搭配**：只分析 blockquote 内已有英文的搭配；写 1-2 组平行搭配 + 中文感受。
**禁止在 Step 2 新增任何英文例句。**
若 source 中该条「例句意图」含**防坑**，必须加一行：
「若按首译思路会错成……；本句用……拉开差距。」

**Step 3 扫句式**：3-5 行，用「谁对谁 / 挂在谁身上 / 逗号后是补充」等白话；说清主谓宾与重心。

**Step 4 读句子**：两层 — 先口语直读（1 句），再考场书面译法（1 句，可省略极简单句）。

篇幅上限：每条 fool 总计不超过 Step1-4 各段之和约 15 行中文；越往后越不能偷懒。

...
src-we/
├── YYYY/                    # 按发布年份存放转换结果
├── questions/YYYY/          # “阅读每日一题”转换结果
├── _inbox/                  # 待转换 HTML
└── _tools/                  # 转换脚本及说明
```

`questions/2025/` 收录 `_inbox/2025/每日一题/` 下全部 112 篇 HTML 的 Markdown
转换结果，文件名与原 HTML 一致，仅扩展名改为 `.md`。标准题目将答案与讲解折叠，
便于先独立做题；周复习、题型汇总和学习建议等非标准文章则按原文顺序保留文字、
重点样式及主要图片。
